...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${t('autorskaTvorbaTitle')} — PRAHO! Project`,
  }
}

export default async function AutorskaTvorbaPage({ params }: Props) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('portfolio')

  const items = getPortfolioByCategory('autorska-tvorba')

  return (
    <main className="py-16 md:py-24">
      <Container>
        <H1 className="mb-4">{t('autorskaTvorbaTitle')}</H1>
        <p className="mb-12 max-w-[var(--width-content)] text-text-muted">
          {t('autorskaTvorbaIntro')}
        </p>
